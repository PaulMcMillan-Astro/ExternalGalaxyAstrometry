...
def filterUsingProperMotions(data,a0=a0vdM,d0=d0vdM,
            ...
            defaultRadiusCutInitialFilter = np.sin(np.deg2rad(3)) # Radius cut for initial sample
                ) :
    '''This function needs a lot of explaining'''
    x,y,mux,muy = Spherical2Orthonormal_pandas(data,alpha0deg=a0,delta0deg=d0,convertUncertainties=False)
    ...
    maskpmLMC = ( (dpmx*dpmx*icovar[0][0] + 2*dpmx*dpmy*icovar[0][1] + dpmy*dpmy*icovar[1][1]) < defaultPMChiSq ) 

    pmxcov, pmycov, pmcov, dpmx, dpmy = [],[],[],[], []

    print(len(data[maskpmLMC]),' stars fit initial proper motion cuts')
    maskposition =  ((data['x']*data['x'] + data['y']*data['y'] < defaultRadiusCutInitialFilter*defaultRadiusCutInitialFilter))
    ...
    pmra_decorr = ( np.array(data.pmra) + 
                np.array(data.parallax_pmra_corr)*np.array(data.pmra_error)/np.array(data.parallax_error)
                * (median_parallax-np.array(data.parallax)) )

    pmdec_decorr = ( np.array(data.pmdec) + 
                np.array(data.parallax_pmdec_corr)*np.array(data.pmdec_error)/np.array(data.parallax_error)
                * (median_parallax-np.array(data.parallax)) )
