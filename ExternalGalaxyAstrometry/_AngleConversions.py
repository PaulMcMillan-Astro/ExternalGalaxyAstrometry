...
a0GaiaEDR3 = 81.07
"""a0 from Gaia EDR3 estimate using all data (Gaia Collaboration 2021, Table 5)"""
d0GaiaEDR3 = -69.41
"""d0 from Gaia EDR3 estimate using all data (Gaia Collaboration 2021, Table 5)"""
...
def Spherical2Orthonormal(
    alphadeg,
    deltadeg,
    mualphastar=None,
    mudelta=None,
    mualphastar_error=None,
    mudelta_error=None,
    mualpha_mudelta_corr=None,
    alpha0deg=a0vdM,
    delta0deg=d0vdM,
):
    """Converts angular position, proper motion, and proper motion uncertainty
    to convenient local form
    ...
    if mualphastar is None or mudelta is None:
        return x, y
    if (
        mualphastar_error is None
        or mudelta_error is None
        or mualpha_mudelta_corr is None
    ):
        mux = cosda * mualphastar - sind * sinda * mudelta
        muy = (
            sind0 * sinda * mualphastar
            + (cosd * cosd0 + sind * sind0 * cosda) * mudelta
        )
        return x, y, mux, muy
...
def Spherical2Orthonormal_pandas(
    data, alpha0deg=a0vdM, delta0deg=d0vdM, convertUncertainties=True
):
...
def Orthonormal2Spherical(
    ...
    alphadeg, deltadeg = np.rad2deg(lambda0 + np.arctan(tanLambda)), np.rad2deg(
        np.arcsin(sinPhi)
    )
    ...
    # C' = R C R^T
    # Multiply by TRANSPOSE of R (hence ij, kj not ij, jk)
    CovarianceMatrix_ad = np.einsum(
        "lij,lkj->lik", CovarianceMatrix, InverseRotationMatrix
    )
    # Multiply by R
    CovarianceMatrix_ad = np.einsum(
        "lij,ljk->lik", InverseRotationMatrix, CovarianceMatrix_ad
    )
    mualphastar_error = np.sqrt(CovarianceMatrix_ad[:, 0, 0])
    mudelta_error = np.sqrt(CovarianceMatrix_ad[:, 1, 1])
    mualpha_mudelta_corr = CovarianceMatrix_ad[:, 0, 1] / (
...
def Orthonormal2Internal(
    ...
    XiDotEtaDot2MuxMuyMatrix[:, 0, 0] = (lx - x * (lx * x + ly * y)) * (
        a * x + b * y + z
    )
    XiDotEtaDot2MuxMuyMatrix[:, 0, 1] = (mx - x * (mx * x + my * y + mz * z)) * (
        a * x + b * y + z
    )
    XiDotEtaDot2MuxMuyMatrix[:, 1, 0] = (ly - y * (lx * x + ly * y)) * (
        a * x + b * y + z
    )
    XiDotEtaDot2MuxMuyMatrix[:, 1, 1] = (my - y * (mx * x + my * y + mz * z)) * (
        a * x + b * y + z
    )

    Determinant = (
        XiDotEtaDot2MuxMuyMatrix[:, 0, 0] * XiDotEtaDot2MuxMuyMatrix[:, 1, 1]
    ...
    mu_xy = np.vstack(
        [
            mux + (-vx + x * (vx * x + vy * y + vz * z)) * (a * x + b * y + z),
            muy + (-vy + y * (vx * x + vy * y + vz * z)) * (a * x + b * y + z),
        ]
    ).T
    ...
    MuxMuy2vRvtMatrix = np.einsum(
        "lij,ljk->lik", XiDotEtaDot2vRvtMatrix, MuxMuy2XiDotEtaDotMatrix
    )
    ...
    # C' = R C R^T
    # Multiply by TRANSPOSE of R (hence ij,kj not ij,jk)
    CovarianceMatrix_vRvt = np.einsum(
        "lij,lkj->lik", CovarianceMatrix, MuxMuy2vRvtMatrix
    )
    # Multiply by R
    CovarianceMatrix_vRvt = np.einsum(
        "lij,ljk->lik", MuxMuy2vRvtMatrix, CovarianceMatrix_vRvt
    )
