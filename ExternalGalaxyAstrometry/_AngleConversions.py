...
def Spherical2Orthonormal(alphadeg, deltadeg,
                          ...
                          mualphastar_error=None, mudelta_error=None,
    ...
    if mualphastar is None or mudelta is None:
        return x, y
    if mualphastar_error is None or mudelta_error is None \
    ...
    CovarianceMatrix_xy = np.einsum('lij,lkj->lik', CovarianceMatrix,
                                    RotationMatrix)
    # Multiply by R
    CovarianceMatrix_xy = np.einsum('lij,ljk->lik',  RotationMatrix,
                                    CovarianceMatrix_xy)
...
def Spherical2Orthonormal_pandas(data, alpha0deg=a0vdM, delta0deg=d0vdM,
    ...
    if convertUncertainties:
        return Spherical2Orthonormal(data.ra, data.dec, data.pmra, data.pmdec,
                                     ...
                                     alpha0deg=alpha0deg, delta0deg=delta0deg)
    else:
        return Spherical2Orthonormal(data.ra, data.dec, data.pmra, data.pmdec,
                                     alpha0deg=alpha0deg, delta0deg=delta0deg)
...
def Orthonormal2Spherical(x, y,
    ...
    alphadeg, deltadeg = np.rad2deg(lambda0+np.arctan(tanLambda)), \
        np.rad2deg(np.arcsin(sinPhi))
...
def Orthonormal2Internal(x, y, mux, muy,
    ...
    Determinant = (XiDotEtaDot2MuxMuyMatrix[:, 0, 0]
                   * XiDotEtaDot2MuxMuyMatrix[:, 1, 1]
                   - XiDotEtaDot2MuxMuyMatrix[:, 1, 0]
                   * XiDotEtaDot2MuxMuyMatrix[:, 0, 1])
    ...
    MuxMuy2vRvtMatrix = np.einsum('lij,ljk->lik', XiDotEtaDot2vRvtMatrix,
                                  MuxMuy2XiDotEtaDotMatrix)
