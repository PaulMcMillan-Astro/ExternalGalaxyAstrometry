...
def minus_logLikelihood(mu_sig, x, xCov):
    ...
    determinantFull = covFull[:, 0, 0]*covFull[:, 1, 1] - covFull[:, 0, 1]*covFull[:, 0, 1]
    ...
    xPrime = x-mu
    exponent = np.einsum('li,li->l', xPrime, np.einsum('lij,lj->li', invcovFull, xPrime))
...
def estimate_mean_dispersion_2D(x, xCov):
    ...
    guessParameters = [tmp[0], tmp[1],  # Guess at weighted mean
    ...
    res = minimize(minus_logLikelihood,  # Function to minimize
                   guessParameters,  # initial guess
                   args=(x, xCov))
    return res.x  # mu, sigmaInt


def binDataOnSky(data, angmax=8, statisticBins=40, verbose=True):
    ...
    # x index
    xint = np.array((np.floor((np.rad2deg(data.x)+angmax)*0.5*statisticBins/angmax))).astype(int)

    # y index
    yint = np.array((np.floor((np.rad2deg(data.y)+angmax)*0.5*statisticBins/angmax))).astype(int)
    ...
    for i in range(statisticBins):
        for j in range(statisticBins):
            ...
            if(countBinned[i, j] > 1):
                ...
                Cov[:, 0, 1] = data.mux_error[mask]*data.muy_error[mask]*data.mux_muy_corr[mask]
                Cov[:, 1, 0] = Cov[:, 0, 1]
                ...
                covUncertainty[0, 1] = np.sum(
                    Cov[:, 1, 0] + meanDispersion[2]*meanDispersion[3]*meanDispersion[4])
    ...
    fullBinnedData = {'x': xcentresfull.flatten(),
